...
fn wav_to_specs(bands: usize, frames: usize, wav_path: &PathBuf) -> Result<Vec<Vec<u8>>, Box<dyn Error>> {
    let handle = std::fs::File::open(wav_path)
    ...
    let samples: Vec<i16> = match (spec.sample_format, spec.bits_per_sample) {
        ...
        (SampleFormat::Int, 16) => {
            ...
            wav.samples::<i16>().collect::<Result<Vec<_>, _>>()
                .map_err(|e| format!("Failed to read 16-bit samples from {:?}: {}", wav_path, e))?
        },
        (SampleFormat::Int, bits) if bits > 16 && bits <= 32 => {
            // 24-bit or 32-bit int: read as i32 and scale down to i16
            wav.samples::<i32>().collect::<Result<Vec<_>, _>>()
        ...
        (SampleFormat::Float, _) => {
            ...
            wav.samples::<f32>().collect::<Result<Vec<_>, _>>()
        ...
        _ => {
            return Err(format!("Unsupported sample format {:?} with {} bits for {:?}",
    ...
    let resampled_samples = match (spec.channels == 2, spec.sample_rate != out_spec.sample_rate) {
        (true, true) => {
    ...
    // Iterate over 10-second windows with 5-second offset
    let mut offset = 0;
